...
class CrazyFlie(MultiAgentEnv):
    ...
    def obs_dim(self) -> int:
        # [state(12) | rel_goal(3) | (hit_mask,dist_norm,dx,dy,dz)×n_rays]
        return self.state_dim + 3 + self._params["n_rays"] * 5

    def _get_obs(self, agent_states: AgentState, goal_states: State, obstacles) -> Array:
        """Build per-agent observation for the transformer.
        o_t = [ state(12) | rel_goal(3) | (hit_mask, dist_norm, dx, dy, dz)×n_rays ]
        Encoding matches collect_pretrain_data_crazyflie._build_obs_v2_3d exactly.
        """
        sense_range = self._params["comm_radius"]
        n_rays = self._params["n_rays"]
        get_lidar_fn = jax.vmap(
            ft.partial(
                get_lidar,
                obstacles=obstacles,
                num_beams=n_rays,
        ...
        lidar_xyz = get_lidar_fn(agent_states[:, :3])   # (n_agents, n_rays, 3)

        def encode_agent(agent_pos, lidar_hits):
            hits_sg = jax.lax.stop_gradient(lidar_hits)
            rel = hits_sg - agent_pos[None, :]              # (n_rays, 3)
            dist = jnp.sqrt(jnp.sum(rel ** 2, axis=-1) + 1e-8)
            hit_mask = jnp.less(dist, sense_range).astype(jnp.float32)

            # SDF-based dist_norm when agent is inside a sphere obstacle.
            # Guard: obstacles.center.shape[0] is static (= n_obs); skip when empty.
            if obstacles.center.shape[0] > 0:
                all_sdf = jax.vmap(
                    lambda obs_j: jnp.linalg.norm(agent_pos - obs_j.center) - obs_j.radius
                )(obstacles)
                min_sdf = all_sdf.min()
                is_inside = min_sdf < 0.0
            else:
                min_sdf = jnp.array(jnp.inf)
                is_inside = jnp.array(False)

            dist_norm = jnp.where(
                is_inside,
                jnp.clip(min_sdf / sense_range, -1.0, 0.0),
                jnp.where(hit_mask > 0.5, jnp.clip(dist / sense_range, 0.0, 1.0), 1.0),
            )
            dir_vec = rel / (dist[:, None] + 1e-8)         # unit direction (n_rays, 3)
            encoded = jnp.concatenate(
                [hit_mask[:, None], dist_norm[:, None], dir_vec], axis=-1
            )                                               # (n_rays, 5)
            return encoded.reshape(-1)                      # (n_rays*5,)

        lidar_encoded = jax.vmap(encode_agent)(
            agent_states[:, :3], lidar_xyz)                 # (n_agents, n_rays*5)
        rel_goal = goal_states[:, :3] - agent_states[:, :3]
    ...
    def step_simulator(self, agent_states: AgentState, action: Action) -> AgentState:
        """Pure JAX RK4 step without episode bookkeeping — used by SQP and horizon rollout."""
        return self.agent_step_rk4(agent_states, action)

    def u_nom_from_state(self, agent_states: jnp.ndarray, goal_states: jnp.ndarray) -> Action:
        """LQR nominal action from raw state arrays (no GraphsTuple required)."""
        return jax_vmap(self.u_ref_inner_single)(agent_states, goal_states)
    ...
    def reset(self, key: Array) -> GraphsTuple:
        ...
        initial_obs = self._get_obs(states, goals, obstacles)
    ...
    def forward_graph(self, graph: GraphsTuple, action: Action) -> GraphsTuple:
        ...
        agent_states = graph.type_states(type_idx=0, n_type=self.num_agents)
        goal_states = graph.type_states(type_idx=1, n_type=self.num_agents)
        obs_states = graph.type_states(type_idx=2, n_type=self.n_rays * self.num_agents)
        action = self.clip_action(action)
