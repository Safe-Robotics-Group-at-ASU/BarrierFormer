...
class Rectangle(NamedTuple):
    ...
    def signed_distance(self, point: Pos2d) -> float:
        """Signed distance from point to Rectangle boundary.
        Positive outside, zero on boundary, negative inside.
        Formula: outside_dist + inside_dist (negative component when inside).
        No zero-norm division — safe for autodiff through the CBF.
        """
        rel_x = point[0] - self.center[0]
        rel_y = point[1] - self.center[1]
        dx = jnp.abs(rel_x * jnp.cos(self.theta) + rel_y * jnp.sin(self.theta)) - self.width / 2
        dy = jnp.abs(rel_x * jnp.sin(self.theta) - rel_y * jnp.cos(self.theta)) - self.height / 2
        outside_dist = jnp.sqrt(jnp.maximum(dx, 0.0) ** 2 + jnp.maximum(dy, 0.0) ** 2 + 1e-8)
        inside_dist  = jnp.minimum(jnp.maximum(dx, dy), 0.0)
