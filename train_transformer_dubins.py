"""
train_transformer_dubins.py
============================
Training script for the Transformer-based CBF (BarrierFormer) on DubinsCar.

Identical in structure to train_transformer.py (DoubleIntegrator) — only the
environment class, PARAMS defaults, and log-dir strings differ.

Differences from train_transformer.py:
  - Uses DubinsCar instead of DoubleIntegrator
  - Default --n-rays = 32  (matches pretrained checkpoint obs_dim=134)
  - Log directory string: "DubinsCar" / run prefix: "gcbf_tf_DC_"

State  : [x, y, theta, v]    (state_dim=4)
Action : [omega, acc]         (action_dim=2)
obs_dim: 4 + 2 + n_rays*4    = 134  for n_rays=32

Usage:
    python train_transformer_dubins.py --area-size 4.0
    python train_transformer_dubins.py --area-size 4.0 --num-agents 1 --steps 2000 --debug
    python train_transformer_dubins.py --area-size 4.0 --n-env-train 16 --steps 5000 --name my_run
    python train_transformer_dubins.py --area-size 4.0 --pretrain-ckpt /path/to/params_1000.pkl
"""
...
from barrierformer.env.dubins_car import DubinsCar
...
def train(args):
    print(f"> Running train_transformer_dubins.py")
    ...
    # ── Environments ─────────────────────────────────────────────────────────
    # DubinsCar is instantiated directly so that history_len can be set.
    # Merge CLI overrides into the default PARAMS so unchanged keys keep their defaults.
    env_params = {**DubinsCar.PARAMS, "n_obs": args.n_obs, "n_rays": args.n_rays}
    env = DubinsCar(
    ...
    log_dir     = os.path.join(args.log_dir, "DubinsCar", "gcbf_transformer", f"seed{args.seed}_{start_time}")
    run_name    = (f"gcbf_tf_DC_H{args.horizon}_hist{args.history_len}_{start_time}" if args.name is None else args.name)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train transformer CBF (BarrierFormer) on DubinsCar"
    )
    ...
    parser.add_argument("--n-rays",            type=int,   default=32,
                        help="LiDAR rays per agent (default: 32, matches pretrained obs_dim=134)")
    ...
    parser.add_argument("--max-grad-norm",      type=float, default=2.0,
                        help="Gradient clipping threshold for cbf_mlp and tf_actor (default: 2.0)")
