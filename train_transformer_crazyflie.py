"""
train_transformer_crazyflie.py
================================
Training script for the Transformer-based CBF (BarrierFormer) on CrazyFlie.

Structure mirrors train_transformer_dubins.py — only the environment class,
PARAMS defaults, and log-dir strings differ.

State  : [x,y,z,psi,theta,phi,u,v,w,r,q,p]   (state_dim=12)
Action : [vx_tgt, vy_tgt, vz_tgt, r_tgt]       (action_dim=4)
obs_dim: 12 + 3 + n_rays*5 = 175  for n_rays=32

Usage:
    python train_transformer_crazyflie.py --area-size 4.0
    python train_transformer_crazyflie.py --area-size 4.0 --num-agents 1 --steps 2000 --debug
    python train_transformer_crazyflie.py --area-size 4.0 --n-env-train 16 --steps 5000 --name my_run
    python train_transformer_crazyflie.py --area-size 4.0 \\
        --pretrain-ckpt ../Pretraining-Code-SDF/pretrain_ckpts_CrazyFlie/params_best.pkl
"""
...
from barrierformer.env.crazyflie import CrazyFlie
...
def train(args):
    print(f"> Running train_transformer_crazyflie.py")
    ...
    # ── Environments ─────────────────────────────────────────────────────────
    env_params = {**CrazyFlie.PARAMS, "n_obs": args.n_obs, "n_rays": args.n_rays}
    env = CrazyFlie(
    ...
    log_dir     = os.path.join(args.log_dir, "CrazyFlie", "gcbf_transformer", f"seed{args.seed}_{start_time}")
    run_name    = (f"gcbf_tf_CF_H{args.horizon}_hist{args.history_len}_{start_time}" if args.name is None else args.name)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train transformer CBF (BarrierFormer) on CrazyFlie"
    )
    ...
    parser.add_argument("--area-size",         type=float, required=True,
                        help="Side length of the cubic world (metres)")
    parser.add_argument("--max-step",          type=int,   default=256,
                        help="Max episode length (default: 256)")
    parser.add_argument("--n-rays",            type=int,   default=32,
                        help="LiDAR rays per agent (default: 32, gives obs_dim=175)")
    parser.add_argument("--n-obs",             type=int,   default=0,
                        help="Number of sphere obstacles per episode (default: 0)")
    ...
    parser.add_argument("--pretrain-ckpt",      type=str,   default=None,
                        help="Path to a pretrained transformer params_*.pkl to warm-start from. "
                             "Example: ../Pretraining-Code-SDF/pretrain_ckpts_CrazyFlie/params_best.pkl")
